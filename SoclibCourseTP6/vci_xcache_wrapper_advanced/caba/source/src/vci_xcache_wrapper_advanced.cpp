...
tmpl(void)::transition()
{
   ...
   m_drsp.rdata   = 0;
    ...
    {
        // TO BE COMPLETED
        if( r_dcache_cacheable_save )
        {
            //  stay in DCACHE_WRITE_REQ state if the request is not accepted 
            m_cost_write_frz++;
            break;     
        }
    ...
    case CMD_IDLE:
    {
        ...
        size_t max;
        // COMPLETED
        if ( r_dcache_miss_req && r_wbuf.miss(m_dreq.addr) )
        {
            r_vci_cmd_fsm = CMD_DATA_MISS;
            r_dcache_miss_req = false;
        } 
        // COMPLETED
        else if ( r_icache_miss_req && r_wbuf.miss(m_dreq.addr) )	
        {
    ...
    case RSP_IDLE:
    {
        ...
        {
            r_vci_rsp_cpt = 0;
            if      ( p_vci.trdid.read() == (r_wbuf.getIndex() + (1 << (vci_param::T - 1))) )
                r_vci_rsp_fsm = RSP_DATA_WRITE;
            else if ( p_vci.trdid.read() == read_transaction_type_e::TYPE_DATA_MISS )
                r_vci_rsp_fsm = RSP_DATA_MISS;
            else if ( p_vci.trdid.read() == read_transaction_type_e::TYPE_DATA_UNC )
                r_vci_rsp_fsm = RSP_DATA_UNC;
            else if ( p_vci.trdid.read() == read_transaction_type_e::TYPE_INS_MISS )
                r_vci_rsp_fsm = RSP_INS_MISS;
            else if ( p_vci.trdid.read() == read_transaction_type_e::TYPE_INS_UNC )
...
tmpl(void)::genMoore()
{
    ...
    case CMD_DATA_MISS:
        ...
        p_vci.pktid  = 0;
        // COMPLETED
        p_vci.trdid  = read_transaction_type_e::TYPE_DATA_MISS;
        p_vci.srcid  = m_srcid;
        p_vci.cons   = false;
        p_vci.wrap   = false;
        p_vci.contig = true;
    ...
    case CMD_DATA_UNC:
        ...
        p_vci.pktid  = 0;
        // COMPLETED
        p_vci.trdid  = read_transaction_type_e::TYPE_DATA_UNC;
        p_vci.srcid  = m_srcid;
        p_vci.cons   = false;
        p_vci.wrap   = false;
        p_vci.contig = true;
    ...
    case CMD_INS_MISS:
        ...
        p_vci.pktid  = 0;
        // COMPLETED
        p_vci.trdid  = read_transaction_type_e::TYPE_INS_MISS;
        p_vci.srcid  = m_srcid;
        p_vci.cons   = false;
        p_vci.wrap   = false;
        p_vci.contig = true;
    ...
    case CMD_INS_UNC:
        ...
        p_vci.cmd    = vci_param::CMD_READ;
        p_vci.pktid  = 0;
        p_vci.trdid  = read_transaction_type_e::TYPE_INS_UNC;
        p_vci.srcid  = m_srcid;
        p_vci.cons   = false;
        p_vci.wrap   = false;
        p_vci.contig = true;
