...
#define SEG_KDATA_BASE   0x80100000         // cluster 0 / tgt 0 / protected
#define SEG_KDATA_SIZE   0x00004000

#define SEG_KUNC_BASE    0x80200000         // cluster 0 / tgt 0 / protected
#define SEG_KUNC_SIZE    0x00001000

#define SEG_CODE_BASE    0x30000000         // cluster 3 / tgt 0
#define SEG_CODE_SIZE    0x00004000

#define SEG_DATA_BASE    0x30100000         // cluster 3 / tgt 0
#define SEG_DATA_SIZE    0x00010000

#define SEG_STACK0_BASE  0x00200000         // cluster 0 / tgt 0
#define SEG_STACK1_BASE  0x10200000         // cluster 1 / tgt 0
#define SEG_STACK2_BASE  0x20200000         // cluster 2 / tgt 0
#define SEG_STACK3_BASE  0x30200000         // cluster 3 / tgt 0
...
int _main(int argc, char *argv[])
{
    ...
    xbar[0] = new VciLocalCrossbar<vci_param>( "xbar0", maptab, 0, 2, 2, 0 );
    xbar[1] = new VciLocalCrossbar<vci_param>( "xbar1", maptab, 1, 2, 3, 0 );
    xbar[2] = new VciLocalCrossbar<vci_param>( "xbar2", maptab, 2, 1, 3, 0 );
    xbar[3] = new VciLocalCrossbar<vci_param>( "xbar3", maptab, 3, 1, 3, 0 );

    std::cout << " - crossbars constructed" << std::endl;

    tty     = new VciMultiTty<vci_param>( "tty", IntTab(3, TGTID_TTY), maptab, "tty0", "tty1", "tty2", "tty3", NULL  );

    std::cout << " - tty constructed" << std::endl;

    tim    = new VciTimer<vci_param>( "tim", IntTab(2, TGTID_TIM), maptab, 4 );

    std::cout << " - timer constructed" << std::endl;

    icu    = new VciMultiIcu<vci_param>( "icu", IntTab(1, TGTID_ICU), maptab, 16, 4 );

    std::cout << " - icu constructed" << std::endl;

    ioc    = new VciBlockDevice<vci_param>( "ioc", maptab, IntTab(0, SRCID_DMA), IntTab(0, TGTID_IOC), ioc_filename );

    std::cout << " - ioc constructed" << std::endl;

    dma    = new VciMultiDma<vci_param>( "dma", maptab, IntTab(1, SRCID_DMA), IntTab(1, TGTID_DMA), 16, 4 );
    ...
    // One-cycle latency, 2-depth FIFO interface, simple case
    noc = new VciVgmn<vci_param>( "noc", maptab, 4, 4, 3, 2 );
