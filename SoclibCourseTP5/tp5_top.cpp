...
int _main(int argc, char *argv[])
{
    ...
    if (argc > 1)
    {
        for( int n=1 ; n<argc ; n=n+2 )
        {
            ...
            else
            {
                ...
                std::cout << "   -NPROCS number of processors" << std::endl;
                exit(0);
            }
        }
    }
    ...
    ///////////////////////////////////////////////////////////////
    // VCI Components : nprocs + 2 initiators / 9 targets
    ...
    char *name[nprocs];
    VciXcacheWrapper<vci_param, GdbServer<Mips32ElIss>>* proc[nprocs];
    for (i = 0; i < nprocs; i++) {
        name[i] = new char[16];
        sprintf(name[i], "proc%d", i);
        proc[i] = new VciXcacheWrapper<vci_param,GdbServer<Mips32ElIss>>(name[i], i, maptab, IntTab(i),
    ...
    for (i = 0; i < nprocs; i++) {
        names.push_back("tty"+std::to_string(i));
    }
    ...
    dma = new VciMultiDma<vci_param>("dma", maptab, IntTab(nprocs+1), IntTab(TGTID_DMA), 16, nprocs);
    ...
    ioc = new VciBlockDevice<vci_param>("ioc", maptab, IntTab(nprocs), IntTab(TGTID_IOC), ioc_filename); 

    VciVgsb<vci_param>* bus;
    bus = new VciVgsb<vci_param>("bus", maptab, nprocs + 2, 9);
    ...
    icu->p_irq_in[16]               (signal_false);
    icu->p_irq_in[17]               (signal_false);
    icu->p_irq_in[18]               (signal_false);
    icu->p_irq_in[19]               (signal_false);
    icu->p_irq_in[20]               (signal_false);
    icu->p_irq_in[21]               (signal_false);
    icu->p_irq_in[22]               (signal_false);
    icu->p_irq_in[23]               (signal_false);
    icu->p_irq_in[24]               (signal_false);
    icu->p_irq_in[25]               (signal_false);
    icu->p_irq_in[26]               (signal_false);
    icu->p_irq_in[27]               (signal_false);
    icu->p_irq_in[28]               (signal_false);
    icu->p_irq_in[29]               (signal_false);
    ...
    for ( int n=1 ; n<ncycles ; n++ )
    {
        if( debug && (n > from_cycle) )
        {
            ...
            ram->print_trace();
            for (i = 0; i < nprocs; i++) {
                if( signal_irq_proc[i].read() )  std::cout << "IRQ_PROC" + std::to_string(i)  << std::endl;
                if( signal_irq_tim[i].read()  )  std::cout << "IRQ_TIM"  + std::to_string(i)  << std::endl;
                if( signal_irq_tty[i].read()  )  std::cout << "IRQ_TTY"  + std::to_string(i)  << std::endl;
                if( signal_irq_dma[i].read()  )  std::cout << "IRQ_DMA"  + std::to_string(i)  << std::endl;
            }
